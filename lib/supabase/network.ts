...
export async function getFollows(): Promise<Follow[]> {
  ...
  const { data, error } = await client
    .from('follows')
    .select('follower:profiles!follows_follower_id_fkey(username), following:profiles!follows_following_id_fkey(username), created_at');
...
export async function getInvites(): Promise<Invite[]> {
  ...
  const { data, error } = await client
    .from('invites')
    .select('id, code, created_at, group_id, groups(name), invite_acceptances(id)')
  ...
  return (data ?? []).map((row: any) => ({
    ...
    code: row.code,
    createdAt: new Date(row.created_at).toLocaleString(),
    acceptedCount: (row.invite_acceptances ?? []).length
  }));
}
...
export async function acceptInvite(code: string) {
  ...
  const { data: invite, error: inviteError } = await client
    .from('invites')
    .select('id, group_id, created_by, groups(name)')
  ...
  await client.from('notifications').insert([
    {
      ...
      body: `You joined ${invite.groups?.name ?? 'a group'}.`
    },
    {
      profile_id: invite.created_by,
      type: 'invite',
      title: 'Your invite was used',
      body: `Someone joined ${invite.groups?.name ?? 'your group'} using your invite link.`
    }
  ]);
  ...
  await client.from('audit_events').insert([
    {
      ...
      detail: `Accepted invite for ${invite.groups?.name ?? invite.group_id}.`
    },
    {
      profile_id: invite.created_by,
      event_name: 'invite_converted',
      detail: `An invite converted for ${invite.groups?.name ?? invite.group_id}.`
    }
  ]);
}
